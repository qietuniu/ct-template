  ...
  <div class="listWrapper">
    <div class="filterContainer">
      ...
      <van-popup
        ...
        v-if="show"
        position="right"
        :style="{ width: '85%', height:'100%',display:'flex',flexDirection: 'column',overflow:'hidden' }"
      >
        <div class="selectContent">
          <van-cell-group title="水果" :border="false">
            <div :class="tagId == null?'selectItem active':'selectItem'" @click="tagId = null">全部</div>
            <div
...
export default {
  ...
  data() {
    return {
      list: [],
      defaultList: new Array(10).fill({
        name: new Date().toTimeString(),
        url: require('../../../assect/img/1.jpg')
      }),
      loading: false,
      finished: true,
      listQuery: {
    ...
    getList() {
      ...
      setTimeout(() => {
        if (_this.listQuery.pageNo == 1) {
          ...
          _this.listQuery.pageNo++
        } else {
          _this.list.push(...(_this.defaultList))
        }
      ...
      //   } else {
      //     _this.list.push(...res.data.result)
      //   }
      //   // 加载状态结束
      //   _this.loading = false
      //   // 数据全部加载完成
      //   if (_this.listQuery.pageNo < res.data.totalPages) {
      //     _this.listQuery.pageNo++
      //     _this.finished = false
      //   } else {
      //     _this.finished = true
